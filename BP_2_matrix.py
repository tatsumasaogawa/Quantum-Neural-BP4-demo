...
def decode(decoder, errorx: torch.Tensor, errorz: torch.Tensor, ep: float, batch_size=1):
    ...
    messages_cn_to_vn = torch.zeros((batch_size, decoder.m_oc, 2 * decoder.n)).to(decoder.device)
    decoder.batch_size = batch_size

    # initlize VN message
    messages_vn_to_cn, _ = decoder.variable_node_update(messages_cn_to_vn, llr, decoder.weights_vn[0],
    ...
    for i in range(decoder.n_iterations):
        ...
        # variable node update:
        messages_vn_to_cn, Gamma = decoder.variable_node_update(messages_cn_to_vn, llr, decoder.weights_vn[i + 1],
        ...
        loss_array[:, i] = decoder.loss(Gamma)
...
for w in tqdm(range(1, 2)):
    ...
    for j in range(n_batches):
        ...
        with torch.no_grad():
            loss, Gamma = decode(decoder, errorx, errorz, ep0, batch_size)

        syn_match, success = decoder.check_syndrome(Gamma)
